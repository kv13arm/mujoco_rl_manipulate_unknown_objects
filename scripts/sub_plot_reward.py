...
log_high_level_dirs = Path(__file__).resolve().parent.parent.as_posix() + "/models/trained_models"

################################## TRAINING DIR #####################################
args.log_dirs = [log_high_level_dirs + "/dir0_reward/acorn_im_reward_best_model",
                 log_high_level_dirs + "/dir0_reward/acorn_progress_reward_best_model",
                 log_high_level_dirs + "/dir0_reward/bread_crumb_im_reward_best_model",
                 log_high_level_dirs + "/dir0_reward/bread_crumb_progress_reward_best_model",
                 log_high_level_dirs + "/dir0_reward/sand_ball_im_reward_best_model",
                 log_high_level_dirs + "/dir0_reward/sand_ball_progress_reward_best_model",
                 log_high_level_dirs + "/dir0_reward/sugar_cube_im_reward_best_model",
                 log_high_level_dirs + "/dir0_reward/sugar_cube_progress_reward_best_model"
                 ]

plot_title = ["Acorn", "Acorn", "Acorn", "Acorn", "Bread crumb", "Bread crumb", "Bread crumb", "Bread crumb", "Sand ball", "Sand ball", "Sand ball", "Sand ball", "Sugar cube", "Sugar cube", "Sugar cube", "Sugar cube"]
...
plot_color_list = ['C0', 'C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8']
for plot_type in ['timesteps']:
    ...
    for idx, result in enumerate(results):
        ...
        xy_list.append((x, y))
        choice_legend = ["progress reward", "novelty reward"]
        if 'im' in algos[idx]:
            algo_legend.append(choice_legend[1])
        else:
            algo_legend.append(choice_legend[0])
        y_min_max_to_consider.append(False)  # do not consider y-min or max in plot for raw data
        if args.smooth:
            window_size = 600
            smooth_type = 2
            if smooth_type == 1:
    ...
    for i, (x, y) in enumerate(xy_list):
        ...
        if(i%4 == 0):  # reset y_min_max for each subplot
